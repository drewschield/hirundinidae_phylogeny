## This is a modification of CEB's original script. Only the inputs and some minor run specifications
## have changed, but the general analysis workflow and model comparison has not (DRS 09/27/23).

## I (CEB) modified this script somewhat in January 2019 to work with my Hirundinidae dataset. I
## removed code that is extraneous after the update to v1.1, and changed some formatting to 
## make the script easier to work with in R studio.
...
####
# 2018-10-10 update: I have been putting the updates on CRAN/GitHub
# You should use:
...
library(parallel) # some systems/R versions prefer library(parallel) over library(snow), try either
...
#    direct ancestors
# 4. You can use non-ultrametric trees, but BioGeoBEARS will interpret any tips significantly below
#    the top of the tree as fossils!  This is only a good idea if you actually do have fossils in
#    your tree, as in e.g. Wood, Matzke et al. (2013), Systematic Biology.
...
# "trfn" = "path/to/tree"
#trfn = "chr-ha95-pruned.tre"
trfn = "hirundinidae-all-rev1-95-prune.tree"
...
tr = read.tree(trfn)
tr
plot(tr, cex = 0.6, no.margin = F, label.offset = 0.1)
title("Time-calibrated Hirundinidae phylogeny inferred by BEAST from 4,000+ UCE loci", cex.main = 0.7)
axisPhylo() # plots timescale

dev.off() # For some reason, if I don't clear this plot the pdf plotting later doesn't work. (CEB)
...
# "geogfn" = "path/to/data"
geogfn = "./data//swallow_geographic_data_8realms.phy"
...
#
# Check the number of states for this dataset: 7 areas, max. range size of 7
...
# Uncomment files you wish to use in time-stratified analyses:
  ## Use the manual dispersal multiplier line to run the analysis with constrained dispersal. (CEB)
#BioGeoBEARS_run_object$timesfn = "timeperiods.txt"
BioGeoBEARS_run_object$dispersal_multipliers_fn = "./data/swallow_manual_dispersal_multipliers.txt"
...
BioGeoBEARS_run_object$on_NaN_error = -1e50   # returns very low lnL if parameters produce NaN error
                                              # (underflow check)
BioGeoBEARS_run_object$speedup = TRUE         # shorcuts to speed ML search; use FALSE if worried
                                              # (e.g. >3 params)
...
#  section_the_tree(inputs=BioGeoBEARS_run_object, make_master_table=TRUE, plot_pieces=FALSE)
...
runslow = TRUE
#runslow = FALSE
resfn = "Hirundinidae_DEC_constrained_dispersal.Rdata"
if (runslow)
{
...
resfn = "Hirundinidae_DEC_J_constrained_dispersal.Rdata"
runslow = TRUE
#runslow = FALSE
if (runslow)
{
...
####
pdffn = "Hirundinidae_DEC_vs_DEC_J_constrained_dispersal.pdf"
pdf(pdffn, width=8.5, height=11)

####
# Plot ancestral states - DEC
####
analysis_titletxt ="BioGeoBEARS DEC on Hirundinidae with constrained dispersal"
...
res2 = plot_BioGeoBEARS_results(
  results_object, analysis_titletxt, addl_params=list("j"), plotwhat="text", label.offset=0.45,
  tipcex=0.6, statecex=0.6, splitcex=0.6, titlecex=0.8, plotsplits=TRUE,
  cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges)

# Pie chart
plot_BioGeoBEARS_results(
  results_object, analysis_titletxt, addl_params=list("j"), plotwhat="pie", label.offset=0.45,
  tipcex=0.6, statecex=0.6, splitcex=0.6, titlecex=0.8, plotsplits=TRUE,
  cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges)
...
####
analysis_titletxt ="BioGeoBEARS DEC+J on Hirundinidae with constrained dispersal"
...
runslow = TRUE
resfn = "Hirundinidae_DIVALIKE_constrained_dispersal.Rdata"
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
resfn = "Hirundinidae_DIVALIKE_J_constrained_dispersal.Rdata"
runslow = TRUE
if (runslow)
{
...
####
#### PDF plots: DIVALIKE vs. DIVALIKE+J ####
####
pdffn = "Hirundinidae_DIVALIKE_vs_DIVALIKE_J_constrained_dispersal.pdf"
pdf(pdffn, width=8.5, height=11)

####
# Plot ancestral states - DIVALIKE
####
analysis_titletxt ="BioGeoBEARS DIVALIKE on Hirundinidae with constrained dispersal"
...
####
analysis_titletxt ="BioGeoBEARS DIVALIKE+J on Hirundinidae with constrained dispersal"
...
runslow = TRUE
resfn = "Hirundinidae_BAYAREALIKE_constrained_dispersal.Rdata"
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
# Under BAYAREALIKE+J, the max of "j" should be 1, not 3 (as is default in DEC+J)
# or 2 (as in DIVALIKE+J)
...
resfn = "Hirundinidae_BAYAREALIKE_J_constrained_dispersal.Rdata"
runslow = TRUE
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
...
####
#### PDF plots: BAYAREALIKE vs. BAYAREALIKE+J ####
####
pdffn = "Hirundinidae_BAYAREALIKE_vs_BAYAREALIKE_J_constrained_dispersal.pdf"
pdf(pdffn, width=8.5, height=11)

####
# Plot ancestral states - BAYAREALIKE
####
analysis_titletxt ="BioGeoBEARS BAYAREALIKE on Hirundinidae with constrained dispersal"
...
####
analysis_titletxt ="BioGeoBEARS BAYAREALIKE+J on Hirundinidae with constrained dispersal"
...
res2 = extract_params_from_BioGeoBEARS_results_object(
  results_object=resDEC, returnwhat="table", addl_params=c("j"), paramsstr_digits=4)
# DEC+J, alternative model for Likelihood Ratio Test (LRT)
res1 = extract_params_from_BioGeoBEARS_results_object(
  results_object=resDECj, returnwhat="table", addl_params=c("j"), paramsstr_digits=4)
...
res2 = extract_params_from_BioGeoBEARS_results_object(
  results_object=resDIVALIKE, returnwhat="table", addl_params=c("j"), paramsstr_digits=4)
# DIVALIKE+J, alternative model for Likelihood Ratio Test (LRT)
res1 = extract_params_from_BioGeoBEARS_results_object(
  results_object=resDIVALIKEj, returnwhat="table", addl_params=c("j"), paramsstr_digits=4)
...
res2 = extract_params_from_BioGeoBEARS_results_object(
  results_object=resBAYAREALIKE, returnwhat="table", addl_params=c("j"), paramsstr_digits=4)
# BAYAREALIKE+J, alternative model for Likelihood Ratio Test (LRT)
res1 = extract_params_from_BioGeoBEARS_results_object(
  results_object=resBAYAREALIKEj, returnwhat="table", addl_params=c("j"), paramsstr_digits=4)
...
# Loads to "restable"
save(restable, file="Hirundinidae_constrained_dispersal_restable.Rdata")
load(file="Hirundinidae_constrained_dispersal_restable.Rdata")

# Loads to "teststable"
save(teststable, file="Hirundinidae_constrained_dispersal_teststable.Rdata")
load(file="Hirundinidae_constrained_dispersal_teststable.Rdata")

# Also save to text files
write.table(restable, file="Hirundinidae_constrained_dispersal_restable.txt", quote=FALSE, sep="\t")
write.table(unlist_df(teststable),
            file="Hirundinidae_constrained_dispersal_teststable.txt", quote=FALSE, sep="\t")
...
AICtable = calc_AICc_column(LnL_vals=restable$LnL, nparam_vals=restable$numparams,
...
# Also save to text files
write.table(restable_AIC_rellike,
            file="Hirundinidae_constrained_dispersal_restable_AIC_rellike.txt", quote=FALSE, sep="\t")
write.table(restable_AICc_rellike,
            file="Hirundinidae_constrained_dispersal_restable_AICc_rellike.txt", quote=FALSE, sep="\t")

# Save with nice conditional formatting
write.table(conditional_format_table(restable_AIC_rellike),
            file="Hirundinidae_constrained_dispersal_restable_AIC_rellike_formatted.txt",
            quote=FALSE, sep="\t")
write.table(conditional_format_table(restable_AICc_rellike),
            file="Hirundinidae_constrained_dispersal_restable_AICc_rellike_formatted.txt",
            quote=FALSE, sep="\t")
...
#
# AIC and AICc model weights are also shown, giving a sense of the relative probability of
# the two models.
#
